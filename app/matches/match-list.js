export default function Match({ match, onSelect, isSelected }) {
    return (
      <div className={`cursor-pointer match mb-4 ${isSelected? 'bg-green-800' : ''} flex flex-col items-center justify-center `}>
        <h3 className="text-xl mb-2" onClick={() => onSelect(match.date)}>Date: {match.date}</h3>
        <p 
          className={`cursor-pointer ${isSelected? 'text-yellow-500' : ''}`} 
          onClick={() => onSelect(match.home_team)}
        >
